...
export async function fetchOutlookMessages(account, { limit = 10 } = {}) {
  ...
  const lastReceivedAt = account?.metadata?.lastReceivedAt
    ? new Date(account.metadata.lastReceivedAt).toISOString()
    : null;

  const filter = lastReceivedAt
    ? `&$filter=receivedDateTime ge ${encodeURIComponent(lastReceivedAt)}`
    : '';

  const url = `${GRAPH_BASE}/me/mailFolders/Inbox/messages?$top=${limit}&$orderby=receivedDateTime desc&$select=sender,from,toRecipients,subject,bodyPreview,body,receivedDateTime,conversationId,internetMessageId${filter}`;
  ...
  if (!data.value || data.value.length === 0) {
    return { messages: [], latestReceivedAt: lastReceivedAt };
  }
  ...
  const messages = data.value.map((item) => ({
    ...
    bodyText: extractPlainText(item.body),
    receivedAt: item.receivedDateTime,
    internetMessageId: item.internetMessageId,
  }));

  const latest = messages.reduce((latestTs, m) => {
  ...
  return {
    messages,
    latestReceivedAt: latest ? new Date(latest).toISOString() : lastReceivedAt,
  };
}
