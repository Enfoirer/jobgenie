...
export async function fetchOutlookMessages(account, { limit = 10 } = {}) {
  ...
  const url = `${GRAPH_BASE}/me/mailFolders/Inbox/messages?$top=${limit}&$orderby=receivedDateTime desc&$select=sender,from,toRecipients,subject,bodyPreview,receivedDateTime,conversationId,internetMessageId${filter}`;
