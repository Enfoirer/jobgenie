        ...
        const messages = await Promise.all(
          ids.map(async (id) => {
            const msgRes = await fetch(
              `${GMAIL_API_BASE}/users/me/messages/${id}?format=full&metadataHeaders=From&metadataHeaders=To&metadataHeaders=Subject&metadataHeaders=Date`,
              { headers: buildAuthorizationHeader(accessToken) }
            );
...
function extractPlainText(payload) {
  // Traverse parts to find text/plain or fallback to text/html
  const stack = [payload];
  while (stack.length) {
